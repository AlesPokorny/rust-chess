...
pub fn get_knight_moves(piece_position: &[usize; 2], friendly_positions: &Vec<[usize; 2]>) -> Vec<(usize, usize)> {
    ...
    for (move_x, move_y) in knight_moves {
        ...
        if (new_x >= 0) & (new_y >= 0) & (new_x <= 7) & (new_y <= 7) {
            ...
            if !friendly_positions.contains(&[new_x, new_y]) {
...
mod test_moves {
    ...
    fn test_get_knight_moves() {
        ...
        let friendly_positions: Vec<[usize; 2]> = vec![
            [0, 0],
            [2, 6],
            [3, 5],
        ];
